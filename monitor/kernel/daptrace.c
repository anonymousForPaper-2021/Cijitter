...
// start hashtable to store addrs
unsigned long targets_addr[10];
int nr_targets_addr[10];
...
int hash_table_size = 0;  //the number of key-value pairs in the hash table!

//string hash function
unsigned int hash_table_hash_str(const unsigned long skey)
{
...
int hash_table_insert(const unsigned long skey, int nvalue)
{
    ...
    HashNode* pNewNode = kmalloc(sizeof(HashNode), GFP_KERNEL);
    pNewNode->sKey = skey;
...
HashNode* hash_table_lookup(const unsigned long skey)
{
    unsigned int pos = hash_table_hash_str(skey) % HASH_TABLE_MAX_SIZE;
    if(hashTable[pos])
    {
    ...
    for(i = 0; i < HASH_TABLE_MAX_SIZE; ++i)
    {
        if(hashTable[i])
        {
            ...
            while(pHead)
            {
                ...
                if(pTemp)
				{
					// sort the nr_accessed
					int flag = -1;
					int i;
					for (i=0; i<10; i++)
					{
						if (pTemp->nValue > nr_targets_addr[i])
						{
							flag = i;
							break;
						}
					...
					for (i=8; i>=flag; i--)
					{
						nr_targets_addr[i + 1] = nr_targets_addr[i];
						targets_addr[i+1] = targets_addr[i];
					}

					nr_targets_addr[flag] = pTemp->nValue;
					targets_addr[flag] = pTemp->sKey;
...
void output_targets_addr(void)
{
        mm_segment_t fs;
        struct file *fp_w;
        fp_w = filp_open(FILE_WRITE, O_RDWR|O_CREAT, 0644);
        if (IS_ERR(fp_w))
        {
                pr_err("faild to open %s\n", FILE_WRITE);
                return;
        }
        ...
        for (i=0; i<10; i++)
        {
                int len = sizeof(targets_addr[i]);
                count = kernel_write(fp_w, &targets_addr[i], len, &pos);
                pos += len;

		int len_acc = sizeof(nr_targets_addr[i]);
                count = kernel_write(fp_w, &nr_targets_addr[i], len_acc, &pos);
                pos += len_acc;
        }
...
static unsigned long aggr_interval = 1000 * 100;
		...
		struct region regions[3])
{
	...
	if (middle != 0)
		...
		end_code = first_gap.start;

	unsigned long div;
	div = (end_code - start)/(3*4096);
	
	regions[0].start = start;
	regions[0].end = regions[0].start + div * 4096;

	regions[1].start = regions[0].end;
        regions[1].end = regions[1].start + div * 4096;

	regions[2].start = regions[1].end;
        regions[2].end = end_code;
}
...
{
	struct pid *id;
	id = find_get_pid(t->pid);
	if(id==NULL)
	{
		pr_err("[Cijitter] pid %d doesn't exist!", t->pid);
		return 0;
	}	
...
static void aggregate(void)
{
	...
	mapia_for_each_task(t) {
		...
		mapia_for_each_region(r, t) {
			...
			start_addr = r->sampling_addr / 4096 * 4096;
			HashNode* pNode = hash_table_lookup(start_addr);
			if (pNode == NULL)
				hash_table_insert(start_addr, r->nr_accesses); 
			else
				pNode->nValue += r->nr_accesses;
